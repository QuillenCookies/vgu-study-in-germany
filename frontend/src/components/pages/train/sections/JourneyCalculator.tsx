...
const JourneyCalculator: React.FC = () => {
    ...
    return (
        <section className="py-8 bg-white-50 dark:bg-gray-950 px-4 h-screen flex items-center justify-center">
            {/* ADDED h-[90vh] to lock the height of the entire dashboard */}
            <div className="w-full max-w-screen-2xl h-[90vh] bg-white dark:bg-gray-900 rounded-3xl shadow-xl overflow-hidden flex flex-col lg:flex-row border border-gray-100 dark:border-gray-800">

                {/* LEFT SIDE: Inputs & Results */}
                <div className="w-full lg:w-5/12 flex flex-col bg-gray-50/50 dark:bg-gray-900 h-full border-r border-gray-200 dark:border-gray-800">

                    {/* HEADER & FORM WRAPPER: shrink-0 prevents it from compressing */}
                    <div className="shrink-0">
                        <div className="p-6 pb-0 mb-4">
                            <h2 className="text-3xl font-extrabold text-[#0a2463] dark:text-yellow-300">Smart Route Explorer</h2>
                            <p className="text-gray-500 dark:text-white-400 mt-1">Plan your journey and check Semesterticket validity.</p>
                        </div>

                        <div className="px-6 pb-6 border-b border-gray-200 dark:border-gray-800">
                            <form onSubmit={handleSearch} className="flex flex-col gap-4">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2 bg-white dark:bg-gray-950 p-4 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm">
                                    <div className="flex flex-col gap-2">
                                        <div className="flex items-center justify-between">
                                            <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase flex items-center gap-1"><Clock className="w-3 h-3" /> Depart In</label>
                                            <div className="flex items-center gap-1 bg-gray-50 dark:bg-gray-800 rounded-lg p-1 border border-gray-200 dark:border-gray-700">
                                                <input type="number" min="0" value={inNextHours} onChange={e => setInNextHours(e.target.value)} className="w-10 bg-transparent text-center font-bold text-[#0a2463] dark:text-blue-100 outline-none" />
                                                <span className="text-gray-400 dark:text-gray-500 text-xs font-medium">h</span>
                                                <input type="number" min="0" max="59" value={inNextMinutes} onChange={e => setInNextMinutes(e.target.value)} className="w-10 bg-transparent text-center font-bold text-[#0a2463] dark:text-blue-100 outline-none" />
                                                <span className="text-gray-400 dark:text-gray-500 text-xs font-medium">m</span>
                                            </div>
                                        </div>
                                        <div className="flex items-center justify-between pt-2 border-t border-gray-100 dark:border-gray-800">
                                            <label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase flex items-center gap-1"><Globe2 className="w-3 h-3" /> Timezone</label>
                                        ...
                                        <div className="text-right text-[10px] text-gray-400 dark:text-gray-500 font-medium">Time: <span className="text-[#0a2463] dark:text-blue-200">{previewTimeStr}</span></div>
                                    </div>
                                    <div className="flex flex-col gap-3 justify-center pl-4 border-l border-gray-100 dark:border-gray-800">
                                        <div className="flex items-center justify-between"><label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Transfers</label>
                                            <select value={maxTransfers} onChange={e => setMaxTransfers(e.target.value)} className="bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg px-2 py-1 text-sm font-medium text-[#0a2463] dark:text-white outline-none">
                                        ...
                                        <div className="flex items-center justify-between"><label className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">Bike</label>
                                            <button type="button" onClick={() => setBikeFriendly(!bikeFriendly)} className={`p-1.5 rounded-lg transition-colors ${bikeFriendly ? 'bg-orange-100 text-[#f97316] dark:bg-orange-900/40' : 'bg-gray-50 dark:bg-gray-800 text-gray-400 border border-gray-200 dark:border-gray-700'}`}><Bike className="w-5 h-5" /></button>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {apiOffline && (
                                    <div className="mt-4 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-2xl flex items-start gap-3 animate-in fade-in slide-in-from-top-2">
                                        <AlertTriangle className="w-5 h-5 text-red-600 dark:text-red-400 shrink-0 mt-0.5" />
                                        <div>
                                            <h4 className="text-sm font-bold text-red-800 dark:text-red-300">Transport API Offline</h4>
                                            <p className="text-xs text-red-600 dark:text-red-400/80 leading-relaxed mt-1">
                    ...
                    <div className="flex-1 overflow-y-auto p-6 bg-white dark:bg-gray-950 custom-scrollbar flex flex-col gap-4">
                        {journeys.map((j, idx) => (
                            <div key={`journey-${idx}`} className={`bg-white dark:bg-gray-900 rounded-2xl border transition-all ${expandedJourney === idx ? 'border-[#f97316] shadow-md dark:border-orange-500' : 'border-gray-200 dark:border-gray-800 hover:border-blue-300 dark:hover:border-blue-700 shadow-sm'}`}>
                                <div className="p-5 cursor-pointer" onClick={() => setExpandedJourney(expandedJourney === idx ? null : idx)}>
                                    <div className="flex justify-between items-start mb-3">
                                        <div>
                                            <h3 className="font-extrabold text-xl text-[#0a2463] dark:text-blue-200">
                                        ...
                                        <div className="text-right">
                                            <span className={`font-bold text-xl ${estimateCost(j).amount === 0 ? 'text-green-600 dark:text-green-400' : 'text-[#f97316]'}`}>
                                                {estimateCost(j).amount === 0 ? '€0.00' : `~€${estimateCost(j).amount.toFixed(2)}`}
                                            </span>
                                            <p className="text-[10px] text-gray-400 dark:text-gray-500 uppercase font-bold tracking-wide">{estimateCost(j).type}</p>
                                        </div>
                                    </div>
                                    <div className="flex gap-1 h-2 rounded-full bg-gray-100 dark:bg-gray-800">
                                        {j.legs.map((l: any, i: number) => <div key={`bar-${idx}-${i}`} className={`h-full flex-1 ${l.line?.product === 'nationalExpress' ? 'bg-[#f97316]' : l.line ? 'bg-[#0a2463] dark:bg-blue-600' : 'bg-gray-300 dark:bg-gray-600'}`}></div>)}
                                    </div>
                                </div>

                                {expandedJourney === idx && (
                                    <div className="p-5 border-t border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-950 flex flex-col gap-5 rounded-b-2xl">
                                        ...
                                        {j.legs.map((leg: any, lIdx: number) => (
                                            <div key={`leg-${idx}-${lIdx}`} className="relative pl-8 border-l-2 border-dotted border-gray-300 dark:border-gray-700 ml-2 pb-4 last:border-transparent last:pb-0">
                                                <div className="absolute w-4 h-4 bg-white dark:bg-gray-900 border-4 border-[#0a2463] dark:border-blue-600 rounded-full -left-[9px] -top-1"></div>
                                                <div className="flex justify-between items-center mb-2">
                                                    <span className="font-bold text-gray-800 dark:text-gray-200 text-sm">{leg.origin?.name}</span>
                                                    <span className="text-xs font-bold text-gray-500 dark:text-gray-400">{new Date(leg.plannedDeparture || leg.departure).toLocaleTimeString([], { timeZone, hour: '2-digit', minute: '2-digit' })}</span>
                                                </div>
                                                <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl p-3 my-2 shadow-sm">
                                                    <div className="flex justify-between items-center">
                                                        <span className={`px-2 py-1 rounded-md text-xs font-bold text-white ${leg.line?.product === 'nationalExpress' ? 'bg-[#f97316]' : leg.line ? 'bg-[#0a2463] dark:bg-blue-600' : 'bg-gray-500 dark:bg-gray-700'}`}>
                                                            {leg.line?.name || 'Walk'}
                                                        </span>
                                                        <span className="text-xs font-bold text-gray-500 dark:text-gray-400">{getDuration(leg.plannedDeparture || leg.departure, leg.plannedArrival || leg.arrival)} min</span>
                                                    </div>
                                                    {leg.direction && <div className="text-xs text-gray-600 dark:text-gray-400 font-medium mt-2">To: {leg.direction}</div>}
                                                </div>
                                                <div className="mt-2 flex justify-between items-center">
                                                    <span className="font-bold text-gray-800 dark:text-gray-200 text-sm">{leg.destination?.name}</span>
                                                    <span className="text-xs font-bold text-gray-500 dark:text-gray-400">{new Date(leg.plannedArrival || leg.arrival).toLocaleTimeString([], { timeZone, hour: '2-digit', minute: '2-digit' })}</span>
                                                </div>
                                            </div>
                                        ))}
