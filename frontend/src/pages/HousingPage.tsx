...
const legalTips = [
  { title: 'Mietvertrag (Rental Contract)', icon: <FileText className="w-5 h-5" />, color: 'bg-blue-50 dark:bg-blue-950/30 border-blue-200 dark:border-blue-800/50 text-blue-800 dark:text-blue-300', desc: 'Always get a written contract. Key items: rent amount (Kaltmiete vs Warmmiete), notice period (usually 3 months), deposit cap (max 3 months cold rent), and Nebenkosten (utility costs).' },
  { title: 'Anmeldung (Registration)', icon: <Home className="w-5 h-5" />, color: 'bg-green-50 dark:bg-green-950/30 border-green-200 dark:border-green-800/50 text-green-800 dark:text-green-300', desc: 'Legally required within 14 days of moving in. Visit the Einwohnermeldeamt (residents\' registration office) with your rental contract and passport. Critical for opening a bank account and getting a student visa.' },
  { title: 'Kaution (Deposit)', icon: <Shield className="w-5 h-5" />, color: 'bg-amber-50 dark:bg-amber-950/30 border-amber-200 dark:border-amber-800/50 text-amber-800 dark:text-amber-300', desc: 'Legally capped at 3 months\' cold rent. Must be returned within 3–6 months after move-out, minus any deductions for damages. Keep all receipts and document the apartment\'s condition with photos at move-in.' },
  { title: 'Tenant Rights', icon: <Shield className="w-5 h-5" />, color: 'bg-purple-50 dark:bg-purple-950/30 border-purple-200 dark:border-purple-800/50 text-purple-800 dark:text-purple-300', desc: 'German tenants have strong legal protections. Landlords cannot raise rent more than 20% in 3 years (Mietpreisbremse). Contact the Mieterverein (Tenants\' Association) if you have disputes.' },
];
...
const HousingPage: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-[#0B1220]">
      ...
      {/* GDPR Notice */}
      <div className="bg-amber-50 dark:bg-amber-950/30 border-b border-amber-200 dark:border-amber-800/50 py-3 px-4">
      ...
      <section className="py-20 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-14">
            <span className="inline-block mb-3 px-4 py-1.5 rounded-full bg-blue-100 dark:bg-blue-950/30 text-blue-700 dark:text-blue-300 text-sm font-semibold">{tr('housing', 'secDistBadge')}</span>
            <h2 className="text-3xl md:text-4xl font-extrabold text-gray-900 dark:text-white mb-4">{tr('housing', 'secDistTitle')}</h2>
            <p className="text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">{tr('housing', 'secDistDesc')}</p>
          </div>
            ...
            {districts.map((d, idx) => (
              <motion.div key={d.name} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                ...
                <div className="p-6">
                  ...
                  <p className="text-xs text-gray-400 dark:text-gray-500 mb-3 flex items-center gap-1"><MapPin className="w-3 h-3" />{d.coordinates}</p>
                  <p className="text-sm text-gray-600 dark:text-gray-400 mb-4 leading-relaxed">{d.highlight}</p>
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-4">
                    {[
                      ...
                      <div key={item.label} className="bg-gray-50 dark:bg-gray-800 rounded-xl p-2 text-center">
                  ...
                  <div className="bg-blue-50 dark:bg-blue-950/30 rounded-xl p-3 border border-blue-100 dark:border-blue-800/50">
                    <p className="text-xs font-semibold text-blue-800 dark:text-blue-300 mb-1.5">Est. Monthly Living Costs</p>
                    <div className="grid grid-cols-3 gap-2 text-center text-xs">
                      <div><span className="text-blue-600 dark:text-blue-400 font-bold">{d.costs.electricity}</span><br /><span className="text-gray-500 dark:text-gray-400">Electricity</span></div>
                      <div><span className="text-blue-600 dark:text-blue-400 font-bold">{d.costs.water}</span><br /><span className="text-gray-500 dark:text-gray-400">Water</span></div>
                      <div><span className="text-blue-600 dark:text-blue-400 font-bold">{d.costs.food}</span><br /><span className="text-gray-500 dark:text-gray-400">Food</span></div>
                    </div>
                  </div>
                </div>
              </motion.div>
      ...
      <section className="py-16 px-4 bg-white dark:bg-gray-900">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-10">
            <span className="inline-block mb-3 px-4 py-1.5 rounded-full bg-green-100 dark:bg-green-950/30 text-green-700 dark:text-green-300 text-sm font-semibold">{tr('housing', 'secPriceBadge')}</span>
            <h2 className="text-2xl md:text-3xl font-extrabold text-gray-900 dark:text-white mb-3">{tr('housing', 'secPriceTitle')}</h2>
          </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {[
                  ...
                  <tr key={row.name} className={i % 2 === 0 ? 'bg-white dark:bg-gray-900' : 'bg-gray-50 dark:bg-gray-800'}>
                    ...
                    <td className="px-5 py-3 font-semibold text-blue-700 dark:text-blue-400">{row.avg}</td>
                    <td className="px-5 py-3 text-gray-600 dark:text-gray-400">{row.sqm}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-3 text-center">* Prices are approximate averages based on aggregated market data. Individual prices may vary.</p>
        </div>
      </section>
      ...
      <section className="py-16 px-4 bg-white dark:bg-gray-900">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-10">
            <span className="inline-block mb-3 px-4 py-1.5 rounded-full bg-blue-100 dark:bg-blue-950/30 text-blue-700 dark:text-blue-300 text-sm font-semibold">{tr('housing', 'secResourcesBadge')}</span>
            <h2 className="text-2xl font-extrabold text-gray-900 dark:text-white mb-3">{tr('housing', 'secResourcesTitle')}</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {resources.map((r) => (
              <a key={r.name} href={r.url} target="_blank" rel="noopener noreferrer"
                className="flex items-start gap-4 p-5 rounded-2xl border border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-800 hover:bg-blue-50 dark:hover:bg-blue-950/30 hover:border-blue-200 dark:hover:border-blue-700 transition-all duration-200 group">
                <div className="text-3xl">{r.icon}</div>
                <div>
                  <h4 className="font-bold text-gray-900 dark:text-white group-hover:text-blue-700 dark:group-hover:text-blue-400 transition-colors">{r.name}</h4>
                  <p className="text-sm text-gray-600 dark:text-gray-400 mt-0.5">{r.desc}</p>
                </div>
              </a>
            ))}
          </div>
          <div className="mt-8 bg-amber-50 dark:bg-amber-950/30 rounded-2xl p-5 border border-amber-200 dark:border-amber-800/50 flex gap-4">
            <Phone className="w-5 h-5 text-amber-600 dark:text-amber-400 flex-shrink-0 mt-0.5" />
            <div>
              <h4 className="font-bold text-amber-800 dark:text-amber-300 mb-1">Need Help? Contact the International Office</h4>
              <p className="text-sm text-amber-700 dark:text-amber-400">Goethe University's International Office offers free housing consultations for new international students. Book your slot at <a href="https://www.uni-frankfurt.de/international" target="_blank" rel="noreferrer" className="underline">uni-frankfurt.de/international</a></p>
            </div>
          </div>
        </div>
      </section>
