...
// ── HERO SEARCH BAR ────────────────────────────────────────────────────────
interface SearchBarProps { onNavigate: (path: string) => void; }
...
const HeroSearchBar: React.FC<SearchBarProps> = ({ onNavigate }) => {
  ...
  return (
    <div className="relative w-full max-w-2xl mx-auto" ref={dropdownRef}>
      <form onSubmit={e => { e.preventDefault(); setShowDropdown(false); onNavigate('/university'); }}
        className="flex w-full items-center bg-white/12 backdrop-blur-xl border border-white/25 rounded-lg overflow-hidden shadow-[0_8px_32px_rgba(0,0,0,0.24)] focus-within:border-white/50 transition-all duration-300">
        <div className="flex items-center pl-4 text-white/60"><Search size={18} /></div>
        ...
        {isLoading && <div className="flex items-center pr-3 text-white/50"><Loader2 size={17} className="animate-spin" /></div>}
        <button type="submit"
          ...
          style={{ backgroundColor: AMBER, color: MIDNIGHT, boxShadow: '0 4px 14px rgba(255,204,0,0.35)' }}
      ...
      <AnimatePresence>
        {showDropdown && (suggestions.cities.length > 0 || suggestions.universities.length > 0) && (
          <motion.div
            ...
            className="absolute top-full mt-2 w-full bg-white rounded-lg shadow-2xl border border-gray-100 overflow-hidden z-50 text-left">
              ...
              {suggestions.universities.length > 0 && (
                <div className="mb-1">
                  ...
                  {suggestions.universities.map((uni, i) => (
                    <button key={`uni-${uni.id || i}`} type="button" onClick={() => handleSelect(uni, 'university')}
              ...
              {suggestions.cities.length > 0 && (
                <div>
                  ...
                  {suggestions.cities.map((city, i) => (
                    <button key={`city-${city.id || i}`} type="button" onClick={() => handleSelect(city, 'city')}
                      className="w-full text-left px-4 py-2.5 text-gray-800 transition-all duration-300 flex items-center gap-3 hover:bg-amber-50">
                      <span className="text-lg">📍</span>
    ...
    <main className="w-full font-sans" style={{ backgroundColor: CREAM }}>
      ...
      <section className="relative w-full flex items-center justify-center min-h-screen overflow-hidden">
        <div className="absolute inset-0 bg-cover bg-center bg-no-repeat scale-105"
          style={{ backgroundImage: `url(${HERO_BG})` }} aria-hidden="true" />
        ...
        <motion.div className="absolute bottom-8 right-8 text-4xl select-none cursor-pointer z-10"
          title="Quack!"
          animate={{ y: [0, -8, 0] }}
        ...
        <motion.div variants={stagger} initial="hidden" animate="show"
          ...
          <motion.span variants={fadeUp}
            className="inline-flex items-center gap-2 mb-6 px-4 py-1.5 rounded-full bg-white/15 backdrop-blur-md border border-white/20 text-white/90 text-[13px] font-medium">
          ...
          <motion.h1 variants={fadeUp}
            ...
            Notes from <span style={{ color: AMBER }}>Die Ente</span>
          </motion.h1>

          <motion.p variants={fadeUp} className="text-lg sm:text-xl text-white/80 mb-4 max-w-xl leading-relaxed">
            {tr('home', 'subHeadline')}
          </motion.p>
          ...
          <motion.div variants={fadeUp} className="mt-5 mb-1">
            <Link to="/community/contributor"
              className="group flex items-center justify-center gap-2 text-[14px] text-white/80 hover:text-white transition-colors">
              ...
              <span className="font-semibold group-hover:underline underline-offset-4"
                style={{ color: AMBER }}>Join the Migration</span>
          ...
          <motion.div variants={stagger} className="flex flex-wrap gap-2.5 mt-6 justify-center">
            {QUICK_TAGS.map(tag => (
              <motion.div key={tag.href} variants={fadeUp}>
                <Link to={tag.href}
                  className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-white/12 hover:bg-white/22 backdrop-blur-sm border border-white/15 text-white text-[13px] font-medium transition-all duration-300 hover:scale-105 hover:border-white/30">
                  <span>{tag.emoji}</span>
