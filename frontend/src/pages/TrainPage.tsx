...
const TrainPage: React.FC = () => {
  ...
  const defaultCity = selectedLocation && selectedLocation.type === 'city' ? selectedLocation.name : 'Frankfurt';

  // API states
  const [stationName, setStationName] = useState(defaultCity);
  ...
  const resolveStation = useCallback(async (query: string, signal?: AbortSignal): Promise<{id: string; name: string; source?: string} | null> => {
    try {
      ...
      if (data.status === 'success' && data.data.length > 0) {
        return data.data[0]; // includes { id, name, source: 'local'|'external' }
      }
    } catch (err: any) {
  ...
  const fetchLiveTrainData = useCallback(async (name: string) => {
    ...
    // State guard: skip if same query already in flight or last fetched
    ...
    try {
      ...
      // If result is from external API, show confirmation prompt (do not auto-save)
  ...
  useEffect(() => {
    fetchLiveTrainData(defaultCity);
    fetchStaticTrainData(defaultCity);
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    if (!q) return;
    // Reset dedup guard so user-initiated re-fetch always goes through
  ...
  const handlePathSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!fromRes || !toRes) {
        setError('Could not resolve stations for journey planning.');
      ...
      // 2. Fetch Journey from external API (v6.db.transport.rest)
      const journeyUrl = `https://v6.db.transport.rest/journeys?from=${fromRes.id}&to=${toRes.id}&results=1&tickets=true`;
      ...
      if (data.journeys && data.journeys.length > 0) {
        ...
        // Map Hafas journey to our internal format
        ...
        const journeyResult: Journey = {
          ...
          fare: fare || undefined,
          isRMV: !fare // Default heuristic: if price is missing in this region, it's likely RMV covered
        };
    ...
    <>
      ...
      <section className="relative bg-[#0a2463] text-white py-20 px-4 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center">
          ...
          <form onSubmit={handleSearch} className="flex max-w-lg mx-auto gap-3 bg-white/10 p-2 rounded-2xl backdrop-blur-lg border border-white/20 shadow-2xl mb-10">
            ...
            <Button type="submit" disabled={isLoading} className="bg-[#f97316] hover:bg-[#ea580c] rounded-xl px-8 h-12 font-bold transition-transform active:scale-95 disabled:opacity-60">
      ...
      {/* ── Journey Planner / Path Suggestion ───────── */}
            ...
            <div className="flex-1 bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100 w-full">
              ...
              <form onSubmit={handlePathSearch} className="flex flex-col gap-4">
                <div className="relative">
                  <span className="absolute left-4 top-3.5 text-gray-400">From</span>
                  <input
                    type="text"
                    value={pathFrom}
                    onChange={(e) => setPathFrom(e.target.value)}
                    placeholder="e.g. Frankfurt Hbf"
                    className="w-full pl-16 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#f97316] transition-all"
                    required
                  />
                </div>
                <div className="relative">
                  <span className="absolute left-4 top-3.5 text-gray-400">To</span>
                  <input
                    type="text"
                    value={pathTo}
                    onChange={(e) => setPathTo(e.target.value)}
                    placeholder="e.g. Goethe Uni"
                    className="w-full pl-16 pr-4 py-3 bg-gray-50 border border-gray-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#f97316] transition-all"
                    required
                  />
                </div>
                <Button type="submit" disabled={isPathLoading || !pathFrom || !pathTo} className="w-full mt-2 h-12 rounded-xl bg-[#0a2463] hover:bg-[#0a2463]/90 text-white font-bold text-lg">
            ...
            <div className="flex-1 w-full flex flex-col justify-center">
              {isPathLoading ? (
                ...
                <div className="text-center p-10 bg-white/50 border border-dashed border-gray-300 rounded-[2rem] h-full flex flex-col items-center justify-center">
                  ...
                  <p className="text-gray-400 font-medium">Enter your start and destination to find the best route</p>
                </div>
              )}
            </div>
          </div>
      ...
      {/* ── Transport Lines Gallery (Mock DB) ────────────────── */}
